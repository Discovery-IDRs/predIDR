...
if [ ! -d out ]; then
  out_dir=/global/scratch/singleton/predIDR/analysis/predictor_eval/disopred3_disprot/out/
  if [ ! -d ${out_dir} ]; then
...
parallel -j $SLURM_CPUS_ON_NODE bash predict.sh ../../../bin/disopred/run_disopred.pl $fasta_dir/{} out/ ::: $(ls $fasta_dir)
