...
# Purpose:
# Examine how increasing the kernal to 100 in combination with a x75 disorder weight
# and x100 epochs affects the performance. Model was rerun to also save initial weights.
...
# sensitivity. Note that this particular model does not follow the former trend.
# While it does have decreased F1 compared to its previous counterpart model 7.1.2
# which used a kernal of 50, it also has increased accuracy, MCC, specificity, and
# precision and decreased sensitivity relative to this previous model. Additionally,
# this series of models all had normal looking training curves (rapid initial growth
# of accuracy and specificity that eventually levels off, something which would be 
# expected from a model that is progressively learning to make better predictions;
# this is in contrast to the overfitting models which demonstrate abnormally good 
# accuracy and specificity right from the start of training). Note that close 
# examination of the training curve graphs for this series seems to show that the
# increases in kernal size from 50 to 200 simply cause the training curves to shift
# increasingly to the right similar to what was observed in models 3.6 through 3.9.
# Due to their relatively balanced performance across all metrics, the models in 
# this series are being considered for further investigation.
...
model_name = "mobidb-pdb_cnn_7_2_2_re"
