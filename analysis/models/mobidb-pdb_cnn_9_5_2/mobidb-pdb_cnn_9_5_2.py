...
# Purpose:
# Examine the performance of a multiheaded model with a 20 kernal head,
# 50 kernal head, and 100 kernal head with a x75 disorder weight trained
# for x100 epochs.

# Architecture:
# disorder weight: x75
# layers: Head 1
#             x2 1D conv layers with 128 filter and 6 kernal
#         Head 2
#             x2 1D conv layers with 128 filter and 10 kernal
#         Head 3
#             x2 1D conv layers with 128 filter and 50 kernal
...
# looked to try to basically do the equivalent of combining a model which had a 
# kernal of 6 with a model that had a kernal of 10 and a model that had a kernal 
# of 50. This model had increased accuracy, MCC, specificity, precision, and F1
# performance and decreased sensitivity performance when compared to the baseline
# model mobidb-pdb_cnn_6_2. Its training curves appear to be normal (features rapid
# initial growth of accuracy and specificity that eventually levels off, something
# which would be expected from a model that is progressively learning to make better
# predictions; this is in contrast to the overfitting models which demonstrate 
# abnormally good accuracy and specificity right from the start of training). This
# model did not seem to demonstrate a notable increase in performance relative to
# other simpler models. As such, while it may be considered for further 
# investigation in the future, it is currently being set aside in favor of more 
# promising models.
...
conv11 = MaskedConv1D(128, 6, padding='same', activation='relu', name='conv1d11')(mask1)
conv12 = MaskedConv1D(128, 6, padding='same', activation='relu', name='conv1d12')(conv11)

# 10 kernal head
mask2 = layers.Masking(mask_value=0, name='mask21')(inputs)
conv21 = MaskedConv1D(128, 10, padding='same', activation='relu', name='conv1d21')(mask2)
conv22 = MaskedConv1D(128, 10, padding='same', activation='relu', name='conv1d22')(conv21)

# 50 kernal head
mask3 = layers.Masking(mask_value=0, name='mask31')(inputs)
conv31 = MaskedConv1D(128, 50, padding='same', activation='relu', name='conv1d31')(mask3)
conv32 = MaskedConv1D(128, 50, padding='same', activation='relu', name='conv1d32')(conv31)

merged = layers.concatenate([conv12,conv22,conv32])
